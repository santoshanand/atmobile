...
class NSEService {
  ...
  var _cookieExpiry = 0;
  var _cookieStr = "";
  final _agent = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Safari/537.36';
  ...
  Future<List<Cookie>?> get httpCookies async {
    ...
    if (_cookieStr == "" || _cookieUsedCount > 10 || _cookieExpiry <= _currentTime) {
  ...
  Future<List<MarketStatusModel>> marketStatus() async {
    var res = await _getData(marketStatusUrl);
    res = res?['marketState'];
    return res?.map<MarketStatusModel>((json) => MarketStatusModel.fromJson(json)).toList();
  }
